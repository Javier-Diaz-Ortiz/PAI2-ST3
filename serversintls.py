# serversintls.py (SIN TLS, EN PUERTO 4443)
...
# Logging
logging.basicConfig(filename="server_notls.log", level=logging.INFO, # Log diferente
                    format="%(asctime)s %(levelname)s %(message)s")

HOST = "0.0.0.0"
# MODIFICACIÓN: Cambiado el puerto para la prueba de rendimiento
PORT = 4443 
...
# --- (Todas las funciones handle_... son idénticas) ---
...
def client_thread(conn, addr):
    # Esta función es idéntica, pero 'conn' es un socket normal
...
# Servidor SIN TLS
# ---------------------------
# MODIFICACIÓN: Función renombrada y simplificada
def start_server_no_tls():
    # MODIFICACIÓN: Eliminada toda la configuración de 'ssl.SSLContext'
    
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM, 0) as sock:
        sock.bind((HOST, PORT))
        sock.listen(512) 
        print(f"Server (NO-TLS) listening on {HOST}:{PORT}")
        logging.info(f"Server (NO-TLS) listening on {HOST}:{PORT}")
        while True:
            client_sock, addr = sock.accept()
            # MODIFICACIÓN: No hay handshake TLS
            # Se pasa el socket 'client_sock' directamente al hilo
            threading.Thread(target=client_thread, args=(client_sock, addr), daemon=True).start()

if __name__ == "__main__":
    start_server_no_tls() # MODIFICACIÓN: Se llama a la nueva función