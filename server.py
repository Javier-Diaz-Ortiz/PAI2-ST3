...
def handle_login_step1(conn, data):
    ...
    if username in blocked_users and time.time() < blocked_users[username]:
        conn.send(json.dumps({"status":"ERROR","msg":"Usuario bloqueado temporalmente"}).encode()); return
    if username in blocked_users:
        blocked_users.pop(username)

    udoc = db.collection(USERS_COLLECTION).document(username).get()
    if not udoc.exists:
        # generic response + record fail
        entry = login_attempts.get(username, {"fails":0})
        entry["fails"] += 1; entry["last_fail"]=time.time(); login_attempts[username]=entry
    ...
    udata = udoc.to_dict()
    salt = udata["salt"]; server_nonce = gen_nonce_hex(16)
...
def handle_login_step2(conn, data):
    username = data.get("username"); client_nonce = data.get("client_nonce"); hmac_hex = data.get("hmac")
    if not username or not client_nonce or not hmac_hex:
        conn.send(json.dumps({"status":"ERROR","msg":"Usuario o contraseña incorrectos"}).encode()); return
    try:
    ...
    verifier = sessions[username]["verifier"]; server_nonce = sessions[username]["server_nonce"]
...
def handle_message(conn, data):
    username = data.get("username"); payload = data.get("payload"); nonce = data.get("nonce"); hmac_hex = data.get("hmac")
    if not username or not payload or not nonce or not hmac_hex:
        conn.send(json.dumps({"status":"ERROR","msg":"Petición inválida"}).encode()); return
    try:
        received_hmac = from_hex(hmac_hex)
    except Exception:
        conn.send(json.dumps({"status":"ERROR","msg":"Petición inválida"}).encode()); return
    if username not in sessions or "client_nonce" not in sessions[username]:
        conn.send(json.dumps({"status":"ERROR","msg":"Usuario no autenticado"}).encode()); return
    # replay
    if sessions[username].get("last_nonce") == nonce:
        conn.send(json.dumps({"status":"ERROR","msg":"Replay detectado"}).encode()); logging.warning(f"TRANSACTION_FAIL user={username} reason=replay_detected"); return

    verifier = sessions[username]["verifier"]; client_nonce = sessions[username]["client_nonce"]; server_nonce = sessions[username]["server_nonce"]
    ...
    if not secure_compare(expected, received_hmac):
        conn.send(json.dumps({"status":"ERROR","msg":"Integridad fallida"}).encode()); logging.warning(f"TRANSACTION_FAIL user={username} reason=integrity_error"); return

    # OK: mark nonce, respond quickly, store async
    sessions[username]["last_nonce"] = nonce
    try:
        doc = {"username": username, "message": payload[:144], "nonce": nonce, "hmac": to_hex(received_hmac), "timestamp": firestore.SERVER_TIMESTAMP}
    except Exception:
        conn.send(json.dumps({"status":"ERROR","msg":"Payload inválido"}).encode()); return
...
def handle_logout(conn, data):
    ...
    if username in sessions:
        sessions.pop(username); conn.send(json.dumps({"status":"LOGOUT_OK"}).encode()); logging.info(f"LOGOUT_OK user={username}")
    else:
        conn.send(json.dumps({"status":"ERROR","msg":"Usuario no tenía sesión activa"}).encode())

# Per-client thread: expects JSON messages per recv
    ...
    try:
        while True:
            ...
            try:
                ...
                conn.send(json.dumps({"status":"ERROR","msg":"JSON inválido"}).encode()); continue
            action = msg.get("action")
            if action == "REGISTER": handle_register(conn, msg)
            elif action == "LOGIN_STEP1": handle_login_step1(conn, msg)
            elif action == "LOGIN_STEP2": handle_login_step2(conn, msg)
            elif action == "MESSAGE": handle_message(conn, msg)
            elif action == "LOGOUT": handle_logout(conn, msg)
            else:
...
def start_tls_server(certfile="certs/server.crt", keyfile="certs/server.key"):
    ...
    # Hardening: disable older protocol versions
    context.options |= ssl.OP_NO_TLSv1 | ssl.OP_NO_TLSv1_1
    # Load server cert
    context.load_cert_chain(certfile=certfile, keyfile=keyfile)
    # prefer server ciphers — keep defaults usually ok; for explicit:
